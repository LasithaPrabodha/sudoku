package com.lasithaprabodha.sudoku.computationlogic


import com.lasithaprabodha.sudoku.domain.Difficulty
import com.lasithaprabodha.sudoku.domain.SudokuNode
import com.lasithaprabodha.sudoku.domain.SudokuPuzzle
import com.lasithaprabodha.sudoku.domain.getHash
import java.util.*
...
internal fun isBasic(puzzle: SudokuPuzzle): Boolean {
    ...
    while (solvable) {
        ...
        puzzle.graph.values.forEach {
            ...
            if (basicSolver(it, puzzle.boundary)) solvable = true
        }
    ...
    while (solvable) {
        ...
        puzzle.graph.values.filter { adjList -> adjList.first.color == 0 }.forEach {
            ...
            if (basicSolver(it, puzzle.boundary)) solvable = true
                ...
                if (advancedSolver(puzzle, superClique, puzzle.boundary)) solvable = true
            }
        }
